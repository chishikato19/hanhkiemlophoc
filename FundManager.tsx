                                                    ...
                                                    <table className="w-full text-left text-[11px] border-collapse">
                                                        ...
                                                        <tbody className="divide-y">
                                                            {activeStudents.sort((a,b) => a.name.localeCompare(b.name)).map((s, idx) => {
                                                                const studentTotal = Object.values(studentPaymentMatrix[s.id]).reduce((sum, val) => sum + val, 0);
                                                                return (
                                                                    <tr key={s.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                                                        ...
                                                                        {selectedIncomesData.map(col => (
                                                                            <td key={col.id} className={`p-3 border-r text-right font-medium ${studentPaymentMatrix[s.id][col.id] > 0 ? 'text-green-600' : 'text-gray-300 italic'}`}>
                                                                                {studentPaymentMatrix[s.id][col.id] > 0 ? formatCurrency(studentPaymentMatrix[s.id][col.id]) : '-'}
                                                                            </td>
                                                                        ))}
