...
import { Check, X, Inbox, Clock, AlertTriangle, RefreshCw, Calendar, CheckCircle, XCircle } from 'lucide-react';
...
const InboxManager: React.FC = () => {
    ...
    const historyReports = reports.filter(r => r.status === 'APPROVED' || r.status === 'REJECTED').sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

    const renderReportItem = (report: PendingReport, isHistory: boolean) => (
        <div key={report.id} className={`border rounded-lg p-4 flex flex-col md:flex-row gap-4 items-start md:items-center hover:shadow-md transition-shadow ${isHistory ? 'bg-gray-100 opacity-80' : 'bg-gray-50 border-indigo-100'}`}>
            <div className="flex-1">
                ...
                <div className="font-bold text-lg text-gray-800 flex items-center gap-2">
                    ...
                    {isHistory && (
                        <span className={`text-xs px-2 py-0.5 rounded flex items-center gap-1 border ${report.status === 'APPROVED' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                            {report.status === 'APPROVED' ? <><CheckCircle size={10}/> Đã duyệt</> : <><XCircle size={10}/> Đã hủy</>}
                        </span>
                    )}
                </div>
                ...
                {!isHistory && report.type === 'ATTENDANCE' && (report.content === AttendanceStatus.LATE || report.content === AttendanceStatus.UNEXCUSED) && (
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6 min-h-[400px]">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                    <Inbox className="text-indigo-600" /> Hộp thư Báo cáo
                </h2>
                <button onClick={handleRefresh} className="flex items-center gap-2 text-sm text-indigo-600 hover:bg-indigo-50 px-3 py-1 rounded transition">
                    <RefreshCw size={16} className={loading ? "animate-spin" : ""} /> Đồng bộ ({pendingReports.length})
                </button>
            </div>

            <div className="space-y-6">
                <section>
                    <h3 className="font-bold text-gray-700 mb-2 flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full"></div> Cần duyệt ({pendingReports.length})</h3>
                    {pendingReports.length === 0 ? (
                        <div className="text-center py-6 text-gray-400 bg-gray-50 rounded border border-dashed">
                            <p>Không có báo cáo nào mới.</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {pendingReports.map(r => renderReportItem(r, false))}
                        </div>
                    )}
                </section>
                ...
                {historyReports.length > 0 && (
                     <section className="pt-6 border-t">
                        <h3 className="font-bold text-gray-500 mb-2 flex items-center gap-2 text-sm uppercase"><div className="w-2 h-2 bg-gray-400 rounded-full"></div> Lịch sử xử lý ({historyReports.length})</h3>
                         <div className="space-y-3 opacity-80">
                            {historyReports.map(r => renderReportItem(r, true))}
                        </div>
                    </section>
                )}
            </div>
