...
import { getStudents, getSettings, saveStudents, getConductRecords } from '../services/dataService';
...
const StoreManager: React.FC = () => {
  ...
  const records = getConductRecords(); // Needed for badge checking
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
       <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <div>
                 ...
                 <p className="text-gray-500 text-sm">Chọn học sinh để vào xem kho đồ hoặc đổi quà.</p>
            </div>
       ...
       <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredStudents.map(student => (
                ...
                >
                    {/* Coin Badge */}
                    <div className="absolute top-2 right-2 bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full text-xs font-bold flex items-center gap-1 shadow-sm">
                        <Coins size={12}/> {student.balance || 0}
                    </div>

                    <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center text-2xl font-bold text-gray-600 mb-3 group-hover:bg-orange-100 group-hover:text-orange-600 transition-colors">
